              ...
              <- case target of
	           HscNothing ->
                       return (emptyMessages,
                               Just ([], nilOL, [], [], NoStubs, hpcInfo, emptyModBreaks, emptyModCCs))
                   _        -> do
                     ...
                     (binds_cvr,ds_hpc_info, modBreaks, modCCs)
                         <- if want_ticks && not (isHsBoot hsc_src)
                              then addTicksToBinds dflags mod mod_loc export_set
                                          (typeEnvTyCons type_env) binds
                              else return (binds, hpcInfo, emptyModBreaks, emptyModCCs)
                     ...
                     initDs hsc_env mod rdr_env type_env $ do
                       do { ds_ev_binds <- dsEvBinds ev_binds
                          ...
                          ; return ( ds_ev_binds
                                   ...
                                   , ds_hpc_info, modBreaks, modCCs) }

        ; case mb_res of {
           Nothing -> return (msgs, Nothing) ;
           Just (ds_ev_binds, all_prs, all_rules, vects0, ds_fords, ds_hpc_info, modBreaks, modCCs) -> do
